...
const CustomTooltip = ({ active, payload }: { active?: boolean; payload?: { payload: { name: string }; value: number }[] }) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-surface/95 border border-border p-4 rounded-xl shadow-xl">
...
export const ScoreChart: React.FC<ScoreChartProps> = ({ categories }) => {
  ...
  return (
    <motion.div 
      ...
      className="w-full h-[400px] bg-surface/80 border border-border rounded-2xl p-6 flex flex-col items-center justify-center"
    >
